...
\code{10} times to the argument \code{1.0}. Note that the closure has access to
...
% Decision diagram: Lush, C++ class, C struct (OpenCV)
% * simple method names (without types)
% * only instantiate required/valid methods (unlike C++ templates)
% * operations supported for arbitrary number of dimensions
% * performance
% * support for arbitrary type-combinations (in NArray/C++ not impossible but
% it is easier to
% work around those limitations)

% * make type diagrams INT(SIGNED, 32), value: Ruby object of matching type
% * pointers (value: Malloc)
% * variables, lambda, lookup, element-wise, ... (optimised building blocks)
% <-> Lambda
% calculus (minimal)

% Shared definitions for datatypes (images) are in Ruby and *not* in C/C++
% -> no static datatype imposed on the software, dynamic Ruby datastructures
% can be used in C/C++ using Ruby's native interface
...
\begin{figure}[htbp]
   \begin{center}
     \resizebox{\columnwidth}{!}{\includegraphics{types}}\\
...
\begin{figure}[htbp]
   \begin{center}
     \resizebox{\columnwidth}{!}{\includegraphics{image}}\\
     \caption{Example of array type\label{fig:image}}
   \end{center}
\end{figure}
% Closures for video input
